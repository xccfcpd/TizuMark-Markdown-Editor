...
### 2.27 用户报障修复：用例图整张报错 + 类图基数丢失（2026-09-24）

用户报告「15.2 用例图渲染不了（Mermaid 报错弹窗）」，顺带体检 15.3 类图也发现一处语义错误。

| 现象 | 根因 | 修法 |
|---|---|---|
| `rectangle 系统 { … }` 的用例图**整张渲染失败**（Mermaid 语法错误） | `rectangle` 不在容器关键字表里 → 被当成**普通节点**；而结尾的 `}` 又**无条件**输出 `end` → **孤立 end** → Mermaid 直接报语法错 | ① 容器表补 `rectangle`/`storage`/`artifact`/`card`（带 `{` → `subgraph`）；② 引入分组栈（`sub` / `plain`），`}` **只与已开启的分组配对**，孤立 `}` 一律忽略；③ `together {` 从 SKIP 移出、按无名分组配对；④ 结尾补齐未闭合的 `subgraph`（源码少写 `}` 也保证配平） |
| 类图 `用户 "1" --> "*" 订单 : 下单` 被转成 `n_1 --> "*" C2 : 下单`（左侧中文类名与基数一起丢失，图上多出一个无意义节点） | 基数解析 `parseSide` 只认「基数在前」（`"1" 用户`）的写法，而 PlantUML **左侧**是「类名在前」（`用户 "1"`） | `parseSide` 同时支持两种顺序：`"card" Name` 与 `Name "card"` |

- 复现与验证：本地用与用户文档逐行一致的样例跑 `plantumlToMermaid`，修复前后对比确认；
...
### 2.29 第七轮审计：按「声明功能」逐项核查（2026-09-24）

方法：把产品声明的 17 项能力（基础 Markdown / 代码高亮 / 数学·物理·化学 / mhchem / 物理单位 /
Mermaid / Graphviz / ECharts 2D / WaveDrom / Unicode 符号 / 公式自动编号 / siunitx / Markmap /
PlantUML / TikZ / plot / Admonition）逐项与实现对照，三路并行审计（数学链路、Markdown·高亮·Unicode·
Admonition、图引擎声明一致性）。
...
| 功能 | 问题（声明 vs 实现） | 修法 |
|---|---|---|
| 数学 / 基础 Markdown | ```` ```math ```` 出现在**普通代码块内部**时被误认成数学围栏开始，且普通围栏的闭合被消费 → **其后整篇文档被吞进代码块**（数据级破坏） | `convertMathFences` 对任意围栏整块跳过，只在非代码块状态下识别数学围栏 |
| 公式自动编号 | 只写 `\tag{1}`、没写 `\label` 的公式不占流水号 → 紧随其后的自动编号**与它撞号**（同页两个 (1)，`\eqref` 指引与视觉不符） | 数字型 `\tag` 抬高流水号（`counter = max(counter, n)`） |
| siunitx / 物理单位 | `\si{kg m}` → `kgm`（**= 毫秒**，语义完全变了）；`kJ//mol`、`kg*m` 原样漏出 | 空白与 `.` 视作单位连接符补 `\,`；`//` → `/`；`*` → `\cdot` |
| 数学 | `.katex-error`（渲染失败）用"次要灰 + 斜体"，与正文几乎无差别 | 改为琥珀色 + 波浪下划线（暗色另配色），失败一眼可见 |
| 数学 | `\(…\)` / `\[…\]` 是**死配置**（配置了但永不生效，看起来像支持） | 从 KaTeX delimiters 移除，并在注释里写明支持 `$…$` / `$$…$$` / ```math |
| Unicode 符号 | `:snowflake:` / `:white_check_mark:` 等 GitHub 常用短码**缺失**（用户验收文档里就用了）；链接文字里的短码被跳过 | 补 29 条短码（现 110+）；`skipTags` 去掉 `'A'` |
| 代码高亮 | **无 hljs 的降级分支**不跳过 mermaid/图表源码 → 行号污染源码、图表渲染失败；`language-c++` 被截成 `c`（按 C 高亮） | 降级分支套用同一套跳过规则；语言名允许 `+ # .` |
| Admonition | `!!! note '标题'`（单引号）**整块失效**；`???-` 不支持；**未知名字**的 `:::` 破坏嵌套配对 | 标题兼容单/双引号；支持 `???-`；任意 `:::` 起始行都计入深度 |
| 基础 Markdown | 只写 `> [!NOTE]`、后面没有正文行的 Callout **整块静默消失** | 空内容也写入 `_END` 标记，保证配对还原 |
| 图引擎（全部） | **点主题按钮不重绘原生引擎**（上一轮只接了 `applyThemeMode`）→ Graphviz 黑线画在暗底上几乎不可见，ECharts/WaveDrom/Markmap 停在旧主题 | `toggleTheme` 里同样调用 `rethemeNativeDiagrams()` |
| ECharts 2D | 声明"点击可放大"但对 canvas 图表**点了毫无反应**（点击被吞） | canvas 图表用 `getDataURL` 走图片灯箱；无 SVG 且无法快照时给明确提示（新增 i18n 键） |
| ECharts 2D | 声明"仅 2D"但 `series.type: 'bar3D'` 只画空白；`tizuHeight` 无上下限（超大值创建巨型 canvas，0 静默回落） | 3D/GL 系列给可读错误；高度夹取到 120–2000 |
| Graphviz | `gv.layout` 是同步阻塞 wasm 调用，**无规模守卫** → 超大 DOT 卡死界面 | 语句数超 4000 时给可读错误而不是假死 |
| 兼容性文档 | 「标题语言」误写"中文"（实现是英文固定）；「标题锚点」误标与 GitHub 一致（实际 `_`→`-` 且合并连续 `-`） | 两行更正为实际行为 |
| ~~主题 / 字体~~ | ~~预览字体变更后，图表缓存里烘焙的仍是旧字体~~ → **该判断有误，修复已在 §2.30 撤销**：`--font-preview` 是静态变量、且 `applyThemeMode → rerenderMermaid` 本就无条件清缓存 | 已删除该 no-op 方法 |

**验证**：本地 208 文件语法 0 错、守卫全过、测试 30 通过（仅 1 个需渲染产物的环境性用例跳过）。

#### 本轮**报告但未改**（按功能）

| 项 | 原因 |
|---|---|
| `[TOC]` 锚点与预览 id 不一致（含链接/短码/重复标题时点击无反应） | TOC 由 **Rust 侧** `generate_toc` 生成（用源码原文 slug、且不去重）。改动在 `src-tauri/src/lib.rs`，而本机**没有 cargo/rustc** 无法编译验证 → 建议在能编译的环境里改用"先剥离行内语法 + 去重"的同款规则 |
| 脚注编号显示 `[^id]` 原文而非连续数字（GitHub 为 `[1]`） | 属显示规则取舍，改动会牵动脚注回归用例 |
| 单行代码块即使开启行号也不显示行号 | 现有用例明确锁定"单行块不包裹 `.code-line`（符合原实现）"，属既有权衡 |
| `classify()` 的 svg 分支 / `SVG_ALIASES` / `CONDITIONAL_TIKZ` / `toSvg` 在应用里不可达（```latex 能力只有单测看见） | 需决定"接上"还是"删掉"；接上会改动 `diagramTypeFromLanguage` 的契约 |
| TikZ / plot 的 `currentColor` 在导出序列化后丢色（暗色长图里可能不可见） | 需在生成 SVG 时按 `isDark` 写显式 `color`（两个转换器 + `renderTikz` 传参） |
| TikZ 逐命令 `scale=` 解析了但未应用 | 需在 item 层做缩放归一与视图边界重算 |
| Word 导出单图转换失败时静默丢图；Markmap 的 `foreignObject` 光栅化表现 | 需要能跑 vendor 的环境实测后再改 |
| WaveDrom dark 皮肤是否真被识别、`@hpcc-js/wasm` 的第 3 参是否被采纳 | 需 `node_modules` 环境冒烟测试 |
| 关于对话框缺 Markmap/d3/docx 归属声明；插入模板只有 6 种引擎（缺 ECharts/WaveDrom/DOT），demo.md 只演示 mermaid | 属文案/入口补齐，可与下轮文案一起做 |
| 数学渲染全同步（上千公式可能卡顿） | 需要视口分批/空闲渲染的调度，属独立改动 |

### 2.30 第九轮（第三轮复核）：挑自己新代码的毛病 + UI 交互与命令层首次审计（2026-09-24）

两路并行：**一路专门挑「第七、八轮」新代码的毛病**，另一路覆盖此前从未审过的 **UI 交互与命令层**
（toolbar / slash / shortcuts / context-menu / dialog-drag-resize / font-picker / notify /
find-replace / word-count / layout）。

#### 我自己引入的问题（6 处，全部已修）

| # | 问题（复核发现） | 修法 |
|---|---|---|
| 1 | `convertMathFences` 用 `trim()` 判定围栏闭合 → **代码块正文里缩进 4 格的 ``` **会把外层围栏提前闭合，后续 ```math 再也不被转换（第七轮引入的回归） | 围栏开/闭一律限制 **≤3 个前导空格**（CommonMark） |
| 2 | `\tag` 抬升只在"非章节模式"生效 → **章节模式下「首个标题之前」的公式照样与 `\tag` 撞号**（同页两个 (1)）；`\tag*{2}`（不带括号的标签）也被当占号 | 抬升条件改为"该公式最终按全局流水号编号"；`\tag*` 排除 |
| 3 | `\si{1.5 m}` → `1.5` 被吃成 `15m`（**数值改坏**）；`\si{100 km}` → `100km` 不补细空格 | 数字（含小数点）作为一个原子；指数标记 `^`/`_` 不再被当"相邻单位" |
| 4 | ECharts「空内容」守卫**误杀合法写法**（timeline 多 option、极坐标 `polar`、`calendar`、`singleAxis`）；且三处校验发生在 `echarts.init` **之后** → 抛错时留下一块空 canvas（短时悬挂实例） | 白名单收全；校验**前移到 init 之前**；3D 判定补 `geo3D/globe/map3D` |
| 5 | Graphviz 空图守卫漏 `<image>`（`shape=none` + `image=` 的图片节点被误判为空图）；规模守卫把**注释里的 `--`** 计入 → 正常规模的图被误报"过大" | 空图判定补 `image/use`；计数前先剥注释 |
| 6 | 我上一轮加的 `__clearDiagramCacheIfFontChanged()` 是**无效修复**：`--font-preview` 是静态 CSS 变量（运行期从不被写），且 `applyThemeMode → rerenderMermaid` 已无条件清缓存 —— 方法整个是 no-op | 删除该方法与调用（§2.29 对应行同时更正） |

#### UI 交互与命令层（首次审计，修 3 处）

| # | 问题 | 修法 |
|---|---|---|
| 7 | 快捷键录制接受**裸键**（如只按 `B`）→ 写进 CodeMirror `extraKeys` 后会**拦住真实打字**（`b` 再也输入不进去） | 录制时要求 Ctrl/Alt 或功能键（F1–F12/方向键等），否则提示并拒绝（新增 `shortcutNeedsModifier` 中英文案） |
| 8 | 全局捕获监听在输入框里吞掉 `Ctrl+Backspace` / `Ctrl+Delete` / `Ctrl+↑↓`（表现为"按键毫无反应"） | 焦点在 input/textarea/contenteditable 时放行这四个键 |
| 9 | 格式工具栏折叠按钮的文案**从不初始化**（要先点一下才出现文字） | `initFormatToolbar` 里初始化即同步文案（与 click 共用同一段） |
| 10 | **i18n 缺键**：`noFileToReload` / `reloaded` 在 `files.js` 里由 `t()` 调用，但中英词典都没有 —— 而 `t()` 对缺失键返回**键名本身**，于是 `t('noFileToReload') || '中文兜底'` 的兜底永不触发，状态栏直接显示 `noFileToReload` 这样的键名 | 两键补进中英词典；并加结构化体检项「`t('key')` ↔ 词典」常态化（本轮：用到 360 键、缺 0） |

#### 报告未改（UI 层，改动面较大或需产品决策）

| 项 | 说明 |
|---|---|
| `applyShortcuts` **只增不删** → 改键/清除后旧键位仍生效（"快捷键改不掉"） | 需重建 extraKeys 基线或记录已注册键并逐个清理 |
| slash 面板被**移动光标**触发（点一下 `/` 右侧就弹）、Esc 关掉后回到 `/` 右侧又弹 | 需改用真实输入信号（`inputRead` / 记录上次值） |
| 设置面板 `×` 用"打开瞬间的整份快照"整体替换 → 吞掉**面板之外**的改动（slash 排序、侧栏开关已落盘却被回滚） | 需把快照收窄为"面板控件对应字段" |
| 拖动对话框标题栏后在遮罩上松手 → 弹框被关闭 | 需 drag 结束置一次性抑制标志 |
| 查找替换：计数走**全文正则**、导航/高亮走**逐行 SearchCursor**（口径不一致）；「替换」与「全部替换」的 `$1` 语义不同 | 需统一到同一套匹配/展开规则 |
| `applySettings()` 不重放布局类设置（「恢复默认 + 应用」后工具栏/侧栏/分栏要重启才生效） | 需收敛出 `applyLayoutSettings()` |
| `dialog-drag-resize` 的 MutationObserver 按**任意 class 变化**重置尺寸/位置 | 应只在"变为可见"那次跃迁重置 |
| `font-picker` 输入框二次点击关闭下拉；`setValue` 同值提前返回导致高亮不刷新 | 小改动，可与其他 UI 修复一起做 |
| `notify` toast 无去重/无上限（同一错误会铺满屏幕）；两个常驻 `setInterval` 无 dispose | 需加签名去重 + 上限 + 句柄清理 |
| `toolbar`/`context-menu` 的重复绑定（当前因整体重建而不累积） | 潜在隐患，记录在案 |

**验证**：本地 208 文件语法 0 错；`unified-math` 59/59（含 +2 条本轮回归）；全量测试 30 通过
