...
行内用法：重力加速度 \SI{9.81}{m/s^2}、质量 \qty{3.5}{\kilo\gram}、单位 \si{\metre\per\second}、\unit{\celsius}、数值 \num{1.23e4}、角度 \ang{45}、温度区间 \SIrange{20}{25}{\celsius}、数值列表 \SIlist{1;2;3}{\metre}、带单位列表 \qtylist{0.5;1.5;2.5}{\kilo\watt}。

`\sisetup` 应被吞掉、不显示原文（下面这行首是它）：

\sisetup{per-mode=symbol} 设置后的同一行仍正常排版：\SI{2.5}{m/s}。
...
### 7.5 WaveDrom：寄存器与赋值图

```wavedrom
{ "reg": { "bits": 4, "lanes": 2, "count": 3 }, "assign": [ "写指针", "读指针" ] }
```
