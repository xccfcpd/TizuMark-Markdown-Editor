...
(function () {
  ...
  const I18N = {
    zh: {
      ...
      eulaDialogTitle: '开源许可协议',
      docxDialogTitle: '导出 DOCX',
      fileSearchDialogTitle: '文件搜索',
      thirdParty: '第三方组件',
      copyright: '版权声明',
      ...
      exportPackagingHint: '正在打包 DOCX（此时可继续编辑或切换标签）…',
      ...
      depUnified: 'Markdown 处理管线（解析 · GFM · HTML 转换 · 安全过滤）',
      depKatex: '数学公式渲染',
      depMhchem: '化学式与化学方程式渲染（\\ce / \\pu）',
      depMermaid: '图表绘制库（流程图 / 时序图 / 甘特图 / 脑图等）',
      depEcharts: '数据图表渲染（```echarts 代码块）',
      depGraphviz: 'DOT 图自动布局渲染（```dot 代码块）',
      depWavedrom: '数字波形图渲染（```wavedrom 代码块）',
      depHtml2canvas: '截图导出',
      depTauri: '桌面应用框架',
      spaces: '空格',
      applying: '正在应用…',
      ...
      editorLargeFileNoHighlight: '⚠ 文档过大：为保证切换标签与滚动流畅，已关闭编辑器语法高亮（预览不受影响）。',
      ...
      exportFullRendering: '正在全量渲染以便导出（约 {lines} 行）…此期间界面无响应属正常，请等待完成（勿重复点击）。',
      exportTooLargeToRender: '文档过大（约 {lines} 行），无法一次性全量渲染导出。请拆分文档后分批导出。',
      exportCancel: '取消导出',
      exportCancelling: '正在取消…',
      exportDiagramPreflight: '正在准备导出：共 {total} 张图表，预计约 {sec} 秒…',
      exportDiagramProgress: '正在处理图表 {done}/{total}（已用 {sec} 秒）…',
      dontRemind: '不再提醒',
    ...
    en: {
      ...
      version: 'Version',
      license: 'License',
      eulaDialogTitle: 'Open Source License',
      ...
      exportPackagingHint: 'Packaging DOCX — you can keep editing or switch tabs…',
      ...
      depUnified: 'Markdown processing pipeline (parse · GFM · HTML · sanitize)',
      depKatex: 'Math formula rendering',
      depMhchem: 'Chemical formulas and equations (\\ce / \\pu)',
      depMermaid: 'Diagram drawing library (flowchart / sequence / gantt / mindmap)',
      depEcharts: 'Data charts (```echarts code blocks)',
      depGraphviz: 'DOT graph auto-layout (```dot code blocks)',
      depWavedrom: 'Digital waveform diagrams (```wavedrom code blocks)',
      ...
      editorLargeFileNoHighlight: '⚠ Large document: editor syntax highlighting is off to keep tab switching and scrolling responsive (preview is unaffected).',
      ...
      exportFullRendering: 'Rendering the whole document for export (~{lines} lines)… the UI will be unresponsive until it finishes. Please wait.',
      exportTooLargeToRender: 'The document is too large (~{lines} lines) to render fully for export. Please split it and export in parts.',
      exportCancel: 'Cancel export',
      exportCancelling: 'Cancelling…',
      exportDiagramPreflight: 'Preparing export: {total} diagrams, about {sec}s…',
      exportDiagramProgress: 'Processing diagram {done}/{total} ({sec}s elapsed)…',
